...
const Secrets = {
    ...
    COOKIE_DIDI: process.env.COOKIE_DIDI, //滴滴Token
};
let Cookies = [];


async function downFile() {
    let response = await axios.get(https://raw.githubusercontent.com/zZPiglet/Task/master/DiDi/DiDi_new.js);
...
async function changeFiele(content, cookie) {
    ...
    content = content.replace(/\$\.read\("#DiDi"\)/g, JSON.stringify(cookie.split("@")[0]))
    content = content.replace(/\$\.read\("#DiDi_city"\)/g, JSON.stringify(cookie.split("@")[1]))
    //console.log(content);
...
async function start() {
    ...
    if (!Secrets.COOKIE_DIDI) {
        console.log("请填写 COOKIE_DIDI 后在继续");
        return;
    }
    if (!Secrets.SyncUrl) {
    ...
    Cookies = Secrets.COOKIE_DIDI.split("\n");
