...
const Secrets = {
    ...
    COOKIE_HDL: process.env.COOKIE_HDL, //海底捞ck
};
let Cookies = [];
...
async function changeFiele(content, cookie) {
    //替换各种信息.
    content = content.replace(/\$cmp\.read\("hilh"\)/g, JSON.stringify(cookie.split("@")[0]))
    content = content.replace(/\$cmp\.read\("hilb"\)/g, JSON.stringify(cookie.split("@")[1]))
    content = content.replace(/\$cmp\.read\("hich"\)/g, JSON.stringify(cookie.split("@")[2]))
    //console.log(content);
...
async function start() {
    ...
    if (!Secrets.COOKIE_HDL) {
        console.log("请填写 COOKIE_HDL 后在继续");
        return;
    }
    if (!Secrets.SyncUrl) {
    ...
    Cookies = Secrets.COOKIE_HDL.split("\n");
